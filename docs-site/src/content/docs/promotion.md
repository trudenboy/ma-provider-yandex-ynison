...
description: Как настроить визуальную презентацию и SEO для репозитория Yandex Music Connect (Ynison)
---

Этот документ описывает, как поддерживается визуальное представление репозитория **Yandex Music Connect (Ynison)** на GitHub и в поиске.
...
- **Badge "Music Assistant"** — динамический shields.io endpoint badge: `https://trudenboy.github.io/ma-provider-tools/badges/yandex_ynison.json`. Cron `update-ma-version-badges.yml` обновляет JSON каждые 4 часа и показывает, в какие каналы MA (stable / beta / dev) включена текущая версия провайдера.
