...
# Dev environment setup for the yandex_ynison provider.
set -euo pipefail
...
if [ -n "$WORKSPACE" ]; then
  ...
  LINK="$WORKSPACE/ma-server/music_assistant/providers/yandex_ynison"
  TARGET="$(python3 -c "import os; print(os.path.relpath('$REPO_ROOT/provider/', '$(dirname "$LINK")'))")"
  rm -f "$LINK"
  ln -s "$TARGET" "$LINK"
  echo "Symlink: yandex_ynison → $TARGET"
  ...
  echo "Provider yandex_ynison linked to workspace. Activate: source $WORKSPACE/.venv/bin/activate"
else
