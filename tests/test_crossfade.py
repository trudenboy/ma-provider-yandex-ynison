...
from provider.crossfade import (
    TailBuffer,
    apply_crossfade,
...
class TestApplyCrossfade:
    ...
    async def test_delegates_to_standard_crossfade(self) -> None:
        ...
        with (
            patch(
                "provider.crossfade.strip_silence",
                new_callable=AsyncMock,
                return_value=fade_out,
            ),
            patch(
                "provider.crossfade.align_audio_to_frame_boundary",
                return_value=fade_out,
            ),
            patch(
            ...
            patch("provider.crossfade.compute_rms_pct", return_value=0.0),
        ):
    ...
    async def test_strip_silence_and_align_called(self) -> None:
        ...
        with (
            patch("provider.crossfade.strip_silence", mock_strip),
            patch("provider.crossfade.align_audio_to_frame_boundary", mock_align),
            patch(
                "music_assistant.controllers.streams.smart_fades.fades.StandardCrossFade.apply",
                _mock_apply,
            ),
    ...
    async def test_noise_detection_aborts_crossfade(self) -> None:
        ...
        with (
            ...
            patch("provider.crossfade.compute_rms_pct", return_value=60.0),
        ):
            collected: list[bytes] = []
            async for chunk in apply_crossfade(fade_out, fade_in, fmt, 5.0, logger):
                collected.append(chunk)
