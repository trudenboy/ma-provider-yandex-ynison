...
async def test_refresh_music_token_success() -> None:
    ...
    with mock.patch("provider.auth.PassportClient.create") as mock_create:
...
def _patched_passport(client: mock.AsyncMock) -> mock._patch[mock.MagicMock]:
    ...
    patcher = mock.patch("provider.auth.PassportClient.create")
...
def _patched_auth_helper() -> tuple[mock._patch[mock.MagicMock], mock.MagicMock]:
    """Patch provider.auth.AuthenticationHelper, returning the helper instance mock."""
    patcher = mock.patch("provider.auth.AuthenticationHelper")
